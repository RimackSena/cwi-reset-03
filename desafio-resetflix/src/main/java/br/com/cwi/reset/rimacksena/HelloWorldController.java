package br.com.cwi.reset.rimacksena;

import br.com.cwi.reset.rimacksena.exceptions.AtorNaoEncontradoExeption;
import br.com.cwi.reset.rimacksena.exceptions.NenhumAtorEncontradoComEsseIdExeption;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

@RestController
@RequestMapping("/hello-world")
public class HelloWorldController {

    @GetMapping
    public String helloWorld() throws NenhumAtorEncontradoComEsseIdExeption, AtorNaoEncontradoExeption {
       throw new AtorNaoEncontradoExeption();
        // return "Minha API resetflix está UP!!!";
    }
}